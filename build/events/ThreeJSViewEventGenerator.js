...
var ThreeJSViewEventGenerator = function ThreeJSViewEventGenerator(scene) {
  _classCallCheck(this, ThreeJSViewEventGenerator);
  ...
  eventGenerator.on('click', function (event, position) {
    ...
    if (closestViewResult) {
      _this.emit('click', event, closestViewResult);
    }
  });
  ...
  eventGenerator.on('mousemove', function (event, position) {
    ...
    if (over) {
      ...
      if (over.view !== (lastOver && lastOver.view)) {
        ...
        _this.emit('mouseenter', event, over);
      }
      lastOver = over;
